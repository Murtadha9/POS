...
const DataTable = ({ data, onRemoveRow, handleSort }) => {
  ...
  const renderCell = (rowIndex, column) => {
    const rowData = data[rowIndex]; // Fix: Get the row data using the rowIndex
    if (
      fieldEditId === `${rowIndex}-${column.key}` ||
      rowData[column.key] === ''
    ) {
      return (
        <input
      ...
      return (
        <div
          ...
          onClick={() => setFieldEditId(`${rowIndex}-${column.key}`)}
        >
          {rowData[column.key]}
        </div>
      );
  ...
  const handleMouseMove = (e) => {
    if (resizingColumn !== null) {
      const newWidth =
        columns[resizingColumn].width + e.clientX - initialResizeX;
      if (newWidth > 20) {
        setColumns((prevColumns) => {
          const newColumns = [...prevColumns];
  ...
  const handleRemoveRow = (id) => {
    ...
    const updatedDataWithNewIds = updatedData.map((row, index) => ({
      ...row,
      id: index + 1,
    }));
